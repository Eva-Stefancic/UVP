...
def razpolovi_seznam(sez):                       
    sredina = len(sez) // 2                      # izračunamo polovico dolžine seznama
    prvi = sez[:sredina]                         # vzamemo prvi del seznama
    drugi = sez[sredina:]                        # vzamemo drugi del seznama
    return prvi, drugi                           # vrnemo par seznamov
...
def zamenjaj_elementa(sez, i, j):                # definiramo funkcijo, ki prejme seznam in dva indeksa
    if i < -len(sez) or i >= len(sez):           # preverimo, ali indeks i ne obstaja
        return sez                               # če indeks i ne obstaja, vrnemo prvotni seznam

    if j < -len(sez) or j >= len(sez):           # preverimo, ali indeks j ne obstaja
        return sez                               # če indeks j ne obstaja, vrnemo prvotni seznam

    nov = sez[:]                                 # naredimo kopijo seznama
    nov[i], nov[j] = nov[j], nov[i]              # zamenjamo elementa na mestih i in j
    return nov                                   # vrnemo nov seznam
...
def porezani_podseznami(sez):                    # definiramo funkcijo, ki prejme seznam
    rezultat = []                                # ustvarimo prazen seznam za rezultat

    for i in range(len(sez) + 1):                # gremo čez vse možne začetke podseznamov
        rezultat.append(sez[i:])                 # dodamo seznam brez prvih i elementov

    return rezultat                              # vrnemo seznam podseznamov
...
def najvecji_element(sez):                       
    ...
    najvecji = sez[0]                            # za največji element najprej vzamemo prvega

    for el in sez[1:]:                           # pregledamo vse ostale elemente
        if el > najvecji:                        # preverimo, ali je trenutni element večji
            najvecji = el                        # če je večji, ga shranimo kot največjega

    return najvecji                              # vrnemo največji element
...
def zdruzi_sezname(sez):                         
    ...
    for podseznam in sez:                        # pregledamo vsak podseznam
        for el in podseznam:                     # pregledamo vsak element v podseznamu
            rezultat.append(el)                  # element dodamo v rezultat
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzY5LCJ1c2VyIjoxMTYyNH0:1w66N9:Ol_LgGCBOtS_SKJlQWw4A8UJPGUWeBFCP8WtOdtDwdk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzcwLCJ1c2VyIjoxMTYyNH0:1w66N9:9JkwbLHqGaJVj2QiJ-JauX1VO2NnzClgRTcNTaThDDw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzcxLCJ1c2VyIjoxMTYyNH0:1w66N9:DzYBTlKYPhcCjUTxkN29Qd0wR2WfiUu7u-MgQMWzj_A"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODEyLCJ1c2VyIjoxMTYyNH0:1w66N9:6NubqMV7pzgLc3ngdAlOYbUu8dJeAJqNtXjM7FKEUys"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDExNiwidXNlciI6MTE2MjR9:1w66N9:mjrCGRqddXImRQI9bDu80AdUB3R1qyeSHa5wm-ZIJsU"
        try:
