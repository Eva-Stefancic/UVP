...
def naivna_resitev(niz):                        
    if niz.count("(") != niz.count(")"):         # preverimo, ali je enako število ( in )
        return False                             
    ...
    return True                                  # če se vsa števila ujemajo, vrnemo True
...
def gnezdeni_oklepaji(niz):                      # definiramo funkcijo, ki prejme niz
    sklad = []                                   # ustvarimo prazen pomožni seznam

    for znak in niz:                             # pregledamo vsak znak v nizu
        if znak in "([{":                        # preverimo, ali je znak uklepaj
            sklad.append(znak)                   # oklepaj dodamo na konec sklada

        elif znak in ")]}":                      # preverimo, ali je znak zaklepaj
            if sklad == []:                      # če je sklad prazen, zaklepaj nima para
                return False                     # zato vrnemo False

            zadnji = sklad[-1]                   # shranimo zadnji oklepaj v skladu

            if znak == ")" and zadnji != "(":    # preverimo, ali se ) ujema z (
                return False                     # če se ne ujema, vrnemo False
            ...
            sklad.pop()                          # če se ujema, zadnji oklepaj odstranimo

    return sklad == []                           # na koncu mora biti sklad prazen



# POVZETEK SNOVI
#
# Metoda count prešteje, kolikokrat se določen znak pojavi v nizu.
#
# Metoda append doda element na konec seznama.
# Metoda pop odstrani zadnji element iz seznama.
#
# Izraz sklad[-1] pomeni zadnji element seznama.
#
# Pri pravilnem gnezdenju oklepajev ni dovolj, da je število oklepajev enako.
# Pomemben je tudi vrstni red oklepajev.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDI2OSwidXNlciI6MTE2MjR9:1w66N9:0syquDtqktjZpCEBqbCnC7hqTXbeqUgH-gc_iUjLI8o"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzM5NCwidXNlciI6MTE2MjR9:1w66N9:njqy_NcqmFGR2o81uFN_twV1ziNZ8XlzEkHHK3OlAjE"
        try:
