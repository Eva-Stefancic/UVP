...
def je_deljivo_s_katerim_od(n, seznam):        
    ...
    return False                               # Če delitelja ne najdemo, vrnemo False.
...
def prastevila_do(n):                          # Definiramo funkcijo, ki sprejme zgornjo mejo.
    prastevila = []                            # Pripravimo prazen seznam za praštevila.

    for stevilo in range(2, n + 1):            # Pregledamo vsa števila od 2 do n.
        if je_prastevilo(stevilo):             # Preverimo, ali je trenutno število praštevilo.
            prastevila.append(stevilo)         # Če je praštevilo, ga dodamo v seznam.

    return prastevila                          # Vrnemo seznam vseh najdenih praštevil.
...
def je_prastevilo(n):                          
    ...
    for delitelj in range(2, n):               # Pregledamo možne delitelje od 2 do n - 1.
        if n % delitelj == 0:                  # Preverimo, ali delitelj deli n.
            return False                       # Če ga deli, n ni praštevilo.

    return True                                # Če ne najdemo delitelja, je n praštevilo.
...
# Povzetek snovi
#
# Praštevilo je naravno število, večje od 1, ki ima natanko dva delitelja.
# Ta dva delitelja sta 1 in število samo.
# Število 1 ni praštevilo.
# Število 2 je najmanjše praštevilo.
# Deljivost preverimo z ostankom pri deljenju.
# V Pythonu ostanek pri deljenju dobimo z operatorjem %.
# Če velja n % delitelj == 0, potem delitelj deli število n.
# Funkcija lahko vrne True, ko pogoj velja.
# Funkcija lahko vrne False, ko pogoj ne velja.
# Z zanko for lahko pregledamo vsa števila v seznamu ali obsegu.
# Funkcija range(2, n + 1) ustvari števila od 2 do n.
# Seznam praštevil gradimo z metodo append.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODA5LCJ1c2VyIjoxMTYyNH0:1w66N9:7Pue1Dh9TbxajOE9aANTuY-G7WWVc94M6C4zW8cxHWg"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODEwLCJ1c2VyIjoxMTYyNH0:1w66N9:Qo1GGlGlOnVX9R9xnQtJQvLzcxTm0MRkROVS7GzHZvM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODExLCJ1c2VyIjoxMTYyNH0:1w66N9:w8BO6Xt9M7cQLSkkcUjN3RfYS50mFVSHiQbsyJor_F4"
        try:
