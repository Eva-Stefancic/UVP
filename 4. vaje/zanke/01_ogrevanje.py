...
def odstrani_presledke(niz):  
    nov_niz = ""              # Ustvarimo prazen niz, v katerega bomo shranjevali znake brez presledkov.
    for znak in niz:          # S pomočjo zanke pregledamo vsak znak v podanem nizu.
        if not znak == " ":   # Preverimo, ali trenutni znak ni presledek.
            nov_niz += znak   # Če znak ni presledek, ga dodamo v nov niz.
    return nov_niz            # Funkcija vrne nov niz, v katerem ni presledkov.
...
def odstrani_veckratne_vprasaje(niz):
    s = ""                                                   # izpraznimo niz
    for i in range(len(niz)):                                # gremo po indeksih ker pogledamo vsakega podebaj
        if i == 0 or niz[i] != "?" or niz[i - 1] != "?":     # prvi element obdržimo ali indeks(element) v nizu ni enak ? ali predhodni element v nizu ni enak ?
            s += niz[i]
    return s 
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDE4OCwidXNlciI6MTE2MjR9:1w3ZC0:4YR4sWb6uvIRVgqB913HNG4ry4a6nQ_U6FkqSRyJyuU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDE4OSwidXNlciI6MTE2MjR9:1w3ZC0:QOrvwmR1mu3UukHuR8UFs8o85r_abjIRHbRsBpbf61I"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDE5MSwidXNlciI6MTE2MjR9:1w3ZC0:ZJugEC1hpEJzWQlOwPjOZl9o8QW0ebQ6FuvvRQAgwz8"
        try:
