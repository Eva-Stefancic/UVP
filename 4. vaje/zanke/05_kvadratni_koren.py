...
def priblizek_po_korakih(n, k):  
    x = n / 2                    # Za začetni približek korena vzamemo polovico števila n.
    for i in range(k):           # Zanko ponovimo k-krat.
        x = (x + n / x) / 2      # Izračunamo nov približek korena po danem postopku.
    return x                     
...
def priblizek_do_natancnosti(n, eps):  
    x = n / 2                          # Za začetni približek korena vzamemo polovico števila n.
    while abs (x ** 2 - n) >= eps:     # Zanko izvajamo, dokler se kvadrat približka od n razlikuje za vsaj eps.
        x = (x + n / x) / 2            # Izračunamo nov približek korena po danem postopku.
    return x                           
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAzLCJ1c2VyIjoxMTYyNH0:1w3ZC0:DflEnmvGmuDhQs5suI42McWV3b_w7BOLc3kabFAmqQk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDA0LCJ1c2VyIjoxMTYyNH0:1w3ZC0:hUIzfDT9157Xk7xjEgS2oDcNiPYGmqugvY81arb_wBM"
        try:
