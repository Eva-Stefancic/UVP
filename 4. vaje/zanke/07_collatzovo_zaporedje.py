...
def naslednji_clen(n):             
    if n % 2 == 0:                 # Preverimo, ali je število n sodo.
        return n // 2              # Če je n sodo, vrnemo polovico števila n.
    else:                          # Če število n ni sodo, je liho.
        return 3 * n + 1           # Če je n liho, vrnemo 3 * n + 1.
...
def dolzina_zaporedja(n):          
    dolzina = 1                    # Začetno število že štejemo kot prvi člen zaporedja.
    
    while n != 1:                  # Zanko izvajamo, dokler ne pridemo do števila 1.
        n = naslednji_clen(n)      # Izračunamo naslednji člen Collatzovega zaporedja.
        dolzina += 1               # Dolžino zaporedja povečamo za 1.
...
def najvecji_clen(n):              
    najvecji = n                   # Za največji člen na začetku vzamemo kar začetno število.
    
    while n != 1:                  # Zanko izvajamo, dokler ne pridemo do števila 1.
        n = naslednji_clen(n)      # Izračunamo naslednji člen Collatzovega zaporedja.
        if n > najvecji:           # Preverimo, ali je novi člen večji od do sedaj največjega.
            najvecji = n           # Če je novi člen večji, ga shranimo kot največjega.
...
def najdaljse_zaporedje(m, n):             
    najdaljse = 0                           # Ustvarimo spremenljivko za dolžino najdaljšega zaporedja.
    
    for stevilo in range(m, n + 1):         # Z zanko pregledamo vsa števila od m do n, vključno z n.
        dolzina = dolzina_zaporedja(stevilo) # Izračunamo dolžino Collatzovega zaporedja za trenutno število.
        if dolzina > najdaljse:             # Preverimo, ali je ta dolžina večja od trenutno največje.
            najdaljse = dolzina             # Če je večja, jo shranimo kot novo največjo dolžino.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODUsInVzZXIiOjExNjI0fQ:1w3ZC0:05gBwwDRsQ8NKOME7-_sh_oSztSbKc-q_dEOqSXE5WQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODYsInVzZXIiOjExNjI0fQ:1w3ZC0:m-DiMJzeZgW9XPKtUvjQwLnOwpUdgqFUQtgy3AiQ5-c"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODcsInVzZXIiOjExNjI0fQ:1w3ZC0:tDyJTatXZdQq64Io_30UHh1SXNVFR1z4BrV30B7W-gQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODgsInVzZXIiOjExNjI0fQ:1w3ZC0:VBesPyBCmMrfb7yHV5fCbEIzbdH6MZQMYuz-o5YqWY8"
        try:
