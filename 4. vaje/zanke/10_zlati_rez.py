...
def priblizek(k):                    
    phi = 1                          # Začetni približek phi_0 je enak 1.
    
    for i in range(k):               # Zanko izvedemo k-krat, da pridemo do k-tega približka.
        phi = naslednji_priblizek(phi) # Iz trenutnega približka izračunamo naslednjega.
...
def natancni_priblizek(eps):          
    prejsnji = 1                      # Za prvi približek vzamemo začetno vrednost 1.
    trenutni = naslednji_priblizek(prejsnji) # Izračunamo naslednji približek za primerjavo.
    
    while abs(trenutni - prejsnji) >= eps: # Zanko izvajamo, dokler je razlika med približkoma prevelika.
        prejsnji = trenutni           # Trenutni približek shranimo kot prejšnjega.
        trenutni = naslednji_priblizek(prejsnji) # Izračunamo nov trenutni približek.
    
    return trenutni                   # Funkcija vrne prvi približek, ki se od prejšnjega razlikuje za manj kot eps.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAwLCJ1c2VyIjoxMTYyNH0:1w3ZC1:Aa6Z-e-d78U8ZWs7Q45BHyn4tTRyAZQD8UimSWLZ9Gk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAxLCJ1c2VyIjoxMTYyNH0:1w3ZC1:cginT-fSqvX1Le2Fxgfpx4n4BFnJiw1l2SY7iT0Qe-I"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAyLCJ1c2VyIjoxMTYyNH0:1w3ZC1:SSrIwrCxEMnMmkOMbfOBbNWwlyoBAUvSIVETcrswkQY"
        try:
