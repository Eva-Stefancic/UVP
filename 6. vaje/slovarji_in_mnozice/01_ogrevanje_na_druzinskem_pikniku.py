...
def druzinski_clani(sladkosnedi):              # Definira funkcijo, ki sprejme slovar sladkosnedov.
    clani = []                                 # Ustvari prazen seznam za imena članov.
    for x in sladkosnedi:                      # Gre čez vse ključe v slovarju.
        clani.append(x)                        # Doda ime člana v seznam.
    return clani                               # Vrne seznam družinskih članov.
...
def zdruzi_sladkosnede(domaci, njihovi):       # Definira funkcijo, ki združi dva slovarja.
    skupaj = {}                                # Ustvari prazen slovar za združene podatke.
    for k in domaci:                           # Gre čez vse člane v slovarju domačih.
        skupaj[k] = domaci[k]                  # Doda člana domačih in njegovo sladico.
    for k in njihovi:                          # Gre čez vse člane v slovarju njihovih.
        if k not in skupaj:                    # Preveri, ali član še ni v združenem slovarju. #da imajo domači prednost
            skupaj[k] = njihovi[k]             # Doda člana njihovih in njegovo sladico.
    return skupaj                              # Vrne združeni slovar sladkosnedov.
...
def nakupovalni_seznam(sladkosnedi, gostje):   # Definira funkcijo, ki sestavi nakupovalni seznam.
    vsi = {}                                   # Ustvari prazen slovar za sladice in količine.
    for oseba in gostje:                       # Gre čez vse goste, ki so potrdili udeležbo.
        if oseba in sladkosnedi:               # Preveri, ali poznamo najljubšo sladico osebe.
            sladica = sladkosnedi[oseba]       # Shrani najljubšo sladico trenutne osebe.
            if sladica in vsi:                 # Preveri, ali je sladica že na seznamu.
                vsi[sladica] += 1              # Poveča količino te sladice za ena.
            else:                              # Izvede se, če sladice še ni na seznamu.
                vsi[sladica] = 1               # Doda sladico na seznam s količino ena.
    return vsi                                 # Vrne slovar sladic in njihovih količin.




# POVZETEK SNOVI
# Slovar hranimo v obliki ključ : vrednost.
# V tej nalogi je ključ ime osebe, vrednost pa njena najljubša sladica.
# Z zanko for po slovarju dobimo njegove ključe.
# Nov seznam ustvarimo z [].
# Nov slovar ustvarimo z {}.
# Element v seznam dodamo z metodo append.
# Vrednost v slovar dodamo ali spremenimo z zapisom slovar[ključ] = vrednost.
# Z izrazom ključ in slovar preverimo, ali ključ obstaja v slovarju.
# Z izrazom ključ not in slovar preverimo, ali ključa ni v slovarju.
# Pri združevanju slovarjev je pomembno, kateri podatki imajo prednost.
# Pri štetju količin najprej preverimo, ali element že obstaja v slovarju.
# Če element že obstaja, njegovo vrednost povečamo.
# Če element še ne obstaja, ga dodamo z začetno vrednostjo.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzNywidXNlciI6MTE2MjR9:1w8aNv:f9_P2NLbYs6uucUhTzkF3YuXzPgpasj09vTA6daMhbI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOCwidXNlciI6MTE2MjR9:1w8aNv:cdSV8XAiaqZOemgBhJCUFysDdcwpXSB7NSJgOs2IMR8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOSwidXNlciI6MTE2MjR9:1w8aNv:Tc5xbbgazaE1yFL5PurvkZoQcfRcKSjkWG5_7Fhw1Lc"
        try:
