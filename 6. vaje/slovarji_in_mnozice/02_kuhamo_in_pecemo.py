...
def pomnozi(recept, celostevilski_faktor):                    # Definiramo funkcijo, ki recept pomnoži z danim faktorjem.
    nov_recept = {}                                           # Ustvarimo prazen slovar, kamor bomo shranili nov recept.
    for sestavina in recept:                                  # Gremo skozi vsako sestavino v receptu.
        nov_recept[sestavina] = recept[sestavina] * celostevilski_faktor  # Količino sestavine pomnožimo s faktorjem in jo shranimo.
    return nov_recept                                         # Vrnemo nov recept s pomnoženimi količinami.
...
def ali_imamo_sestavine(recept, shramba):                     # Definiramo funkcijo, ki preveri, ali imamo dovolj sestavin.
    for sestavina in recept:                                  # Pregledamo vsako sestavino, ki jo zahteva recept.
        if sestavina not in shramba:                          # Preverimo, ali sestavine sploh ni v shrambi.
            return False                                      # Če sestavine nimamo, vrnemo False.
        if shramba[sestavina] < recept[sestavina]:            # Preverimo, ali je količina v shrambi manjša od potrebne količine.
            return False                                      # Če sestavine ni dovolj, vrnemo False.
    return True                                               # Če imamo vse sestavine v zadostnih količinah, vrnemo True.  
...
def kaj_moramo_se_kupiti(recept, shramba):                    # Definiramo funkcijo, ki ugotovi, kaj moramo še kupiti.
    kupi = {}                                                 # Ustvarimo prazen slovar za sestavine, ki jih moramo dokupiti.
    for sestavina in recept:                                  # Pregledamo vsako sestavino v receptu.
        if sestavina not in shramba:                          # Preverimo, ali sestavine ni v shrambi.
            kupi[sestavina] = recept[sestavina]               # Če je nimamo, moramo kupiti celotno potrebno količino.
        else:                                                 # Če sestavino imamo v shrambi, preverimo njeno količino.
            if shramba[sestavina] < recept[sestavina]:        # Preverimo, ali imamo sestavine premalo.
                kupi[sestavina] = recept[sestavina] - shramba[sestavina]  # Izračunamo, koliko sestavine še manjka.
    return kupi                                               # Vrnemo slovar sestavin, ki jih moramo še kupiti.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzkyLCJ1c2VyIjoxMTYyNH0:1w8aNv:dZOkTtKJYgzfBUmch_UYCzMM8_BsQdttqxTaMKcTYVw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzkzLCJ1c2VyIjoxMTYyNH0:1w8aNv:NRGPJ5ATAgd_iFhau40_B20zSdjXP6BlQwYyU25jWvU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzk0LCJ1c2VyIjoxMTYyNH0:1w8aNv:O5f4tn6ksJJqCbI6YXUcvhjRvX8kM8GbQ8MPyzgIUuk"
        try:
