...
def sifriraj(nasa_sifra, beseda):                              # Definiramo funkcijo, ki zašifrira podano besedo.
    rezultat_sifriranja = ""                                   # Ustvarimo prazen niz, v katerega bomo shranili zašifrirano besedo.
    for x in beseda:                                           # Gremo skozi vsako črko v podani besedi.
        rezultat_sifriranja += nasa_sifra[x]                   # Črko zamenjamo z ustrezno črko iz šifre in jo dodamo rezultatu.
    return rezultat_sifriranja                                 # Vrnemo zašifrirano besedo.
...
def ali_je_sifra(slovar):                                      # Definiramo funkcijo, ki preveri, ali slovar predstavlja šifro.
    ...
    if kljuci == vrednosti:                                    # Preverimo, ali so ključi in vrednosti enaki.
        return True                                            # Če so enaki, slovar predstavlja šifro.
    else:                                                      # Če ključi in vrednosti niso enaki.
        return False                                           # Slovar ne predstavlja šifre.


def ali_je_sifra(slovar):                                      # Ponovno definiramo funkcijo za preverjanje šifre.
    return set(slovar) == set(slovar.values())                 # Vrnemo, ali so ključi in vrednosti slovarja enaki.
...
def inverz(sifra):                                             # Definiramo funkcijo, ki vrne obratno šifro.
    if not ali_je_sifra(sifra):                                # Preverimo, ali podani slovar ni veljavna šifra.
        return None                                            # Če šifra ni veljavna, vrnemo None.
    else:                                                      # Če je šifra veljavna.
        slovar = {}                                            # Ustvarimo prazen slovar za inverz šifre.
        for kljuc, vrednost in sifra.items():                  # Gremo skozi vsak par ključa in vrednosti v šifri.
            slovar[vrednost] = kljuc                           # V novi slovar zapišemo obrnjen par.
        return slovar                                          # Vrnemo inverzno šifro.
...
def odsifriraj(sifra, zasifrirano_besedilo):                   # Definiramo funkcijo, ki odšifrira zašifrirano besedilo.
    if not ali_je_sifra(sifra):                                # Preverimo, ali podani slovar ni veljavna šifra.
        return None                                            # Če šifra ni veljavna, vrnemo None.
    
    obrat = {}                                                 # Ustvarimo prazen slovar za obratno šifro.
    for kljuc in sifra:                                        # Gremo skozi vsak ključ v šifri.
        obrat[sifra[kljuc]] = kljuc                            # V obratni slovar shranimo vrednost kot ključ in ključ kot vrednost.
   
    rezultat = ""                                              # Ustvarimo prazen niz za odšifrirano besedilo.
    for crka in zasifrirano_besedilo:                          # Gremo skozi vsako črko v zašifriranem besedilu.
        rezultat += obrat[crka]                                # Črko odšifriramo z obratno šifro in jo dodamo rezultatu.
    
    return rezultat                                            # Vrnemo odšifrirano besedilo.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg4LCJ1c2VyIjoxMTYyNH0:1w8aNv:x-WFrHHgyhwl_uWO3h0oKfwz3Te2Y0F_vOG6nAl34Rg"
        try:
            nasa_sifra = {'Č': 'K', 'A': 'O', 'C': 'Z', 'B': 'M', 'E': 'V',
                          'D': 'C', 'G': 'P', 'F': 'E', 'I': 'B', 'H': 'F',
                          'K': 'I', 'J': 'A', 'M': 'U', 'L': 'H', 'O': 'R',
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg5LCJ1c2VyIjoxMTYyNH0:1w8aNv:vSC05hPJ_2CgaQSWCca0vl0q-X1JyYPar5QfSjrakrU"
        try:
            nasa_sifra = {'Č': 'K', 'A': 'O', 'C': 'Z', 'B': 'M', 'E': 'V',
                          'D': 'C', 'G': 'P', 'F': 'E', 'I': 'B', 'H': 'F',
                          'K': 'I', 'J': 'A', 'M': 'U', 'L': 'H', 'O': 'R',
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzkwLCJ1c2VyIjoxMTYyNH0:1w8aNv:QbyhjHM1XGxE1qxQO67jasV_2hk-GmjkgLcklZ6FMAI"
        try:
            nasa_sifra = {'Č': 'K', 'A': 'O', 'C': 'Z', 'B': 'M', 'E': 'V',
                          'D': 'C', 'G': 'P', 'F': 'E', 'I': 'B', 'H': 'F',
                          'K': 'I', 'J': 'A', 'M': 'U', 'L': 'H', 'O': 'R',
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzkxLCJ1c2VyIjoxMTYyNH0:1w8aNv:0ZD1mLIxrEddYlh7hFaxMbuBW_RBcvGEiU-7OrSdFvU"
        try:
            nasa_sifra = {'Č': 'K', 'A': 'O', 'C': 'Z', 'B': 'M', 'E': 'V',
                          'D': 'C', 'G': 'P', 'F': 'E', 'I': 'B', 'H': 'F',
                          'K': 'I', 'J': 'A', 'M': 'U', 'L': 'H', 'O': 'R',
