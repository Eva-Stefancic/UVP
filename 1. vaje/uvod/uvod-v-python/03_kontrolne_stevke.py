...
def vsota_kvadratov_stevk(n):              # Definira funkcijo, ki sprejme število n.
    enice = n % 10                         # Izračuna zadnjo števko števila, torej enice.
    desetice = (n // 10) % 10              # Izračuna srednjo števko števila, torej desetice.
    stotice = n // 100                     # Izračuna prvo števko tromestnega števila, torej stotice.
    return stotice**2 + desetice**2 + enice**2  # Vrne vsoto kvadratov vseh treh števk.
...
def obrat(n):                                      # Definira funkcijo, ki sprejme tromestno število n.
    enice = n % 10                                # Izračuna zadnjo števko števila, torej enice.
    desetice = (n // 10) % 10                     # Izračuna srednjo števko števila, torej desetice.
    stotice = n // 100                            # Izračuna prvo števko števila, torej stotice.
    return (enice * 100 + desetice * 10 + stotice) # Sestavi novo število tako, da enice postavi na mesto stotic, stotice pa na mesto enic.
...
def dodaj_kontrolno_stevko(sklic):          # Definira funkcijo, ki sprejme 12-mestni sklic.
    sklic = str(sklic)                      # Sklic pretvori v niz, da lahko dostopamo do posameznih števk.

    faktor = 2                              # Nastavi prvi faktor, s katerim pomnožimo števko na mestu enic.
    vsota = 0                               # Pripravi spremenljivko, v katero bomo seštevali produkte.

    for i in range(11, -1, -1):             # Gre čez vseh 12 števk od desne proti levi.
        stevka = int(sklic[i])              # Trenutno števko pretvori iz niza v celo število.
        vsota += stevka * faktor            # K vsoti prišteje trenutno števko, pomnoženo s faktorjem.
        faktor += 1                         # Faktor poveča za 1 za naslednjo števko.

    ostanek = vsota % 11                    # Izračuna ostanek pri deljenju vsote z 11.
    kontrolna = 11 - ostanek                # Izračuna možno kontrolno števko.

    if kontrolna < 10:                      # Preveri, ali je kontrolna števka manjša od 10.
        kontrolna                           # Ta vrstica ne spremeni ničesar; samo navede vrednost spremenljivke.
    else:                                   # Izvede se, če kontrolna števka ni manjša od 10.
        kontrolna = 0                       # Če je kontrolna 10 ali 11, jo nastavi na 0.

    novo_stevilo = sklic + str(kontrolna)   # Na konec sklica doda kontrolno števko.
    return int(novo_stevilo)                # Novo 13-mestno število pretvori v int in ga vrne.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjExNjI0fQ:1wNAxj:ckaLBh5I-_L-TE3TkEUEUKhGGU4dWGOSRchcy_4aMgg"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjExNjI0fQ:1wNAxj:YvhH-fqmz0BoHEYF667biPiok9T01veZHOLv_MQDe0M"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjExNjI0fQ:1wNAxj:ZDjI8k0Bs5svJMIoEeghQB4olr_avZ9NUg8IKKNctmo"
        try:
