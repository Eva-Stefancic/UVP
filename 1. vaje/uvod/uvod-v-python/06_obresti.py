...
def enostavno_obrestovanje(polog, mesecna_obrestna_mera, st_mesecev):  # Definira funkcijo, ki sprejme polog, mesečno obrestno mero in število mesecev.

    if st_mesecev == 0:                                                # Preveri, ali denar ni bil obrestovan noben mesec.
        return float(polog)                                            # Vrne začetni polog kot decimalno število.
    else:                                                              # Izvede se, če je število mesecev večje od 0.
        return float(polog * (1 + mesecna_obrestna_mera * st_mesecev)) # Izračuna vrednost denarja po formuli za enostavno obrestovanje.
...
def obrestno_obrestovanje(polog, mesecna_obrestna_mera, st_mesecev):  # Definira funkcijo, ki sprejme polog, mesečno obrestno mero in število mesecev.
    if st_mesecev == 0:                                               # Preveri, ali denar ni bil obrestovan noben mesec.
        return float(polog)                                           # Vrne začetni polog kot decimalno število.
    else:                                                             # Izvede se, če je število mesecev večje od 0.
        return float(polog * (1 + mesecna_obrestna_mera) ** st_mesecev)  # Izračuna vrednost po formuli za obrestno obrestovanje.
...
def pretvori_v_letno_obrestno_mero(mesecna_obrestna_mera):  # Definira funkcijo, ki sprejme mesečno obrestno mero.

    letna_obrestna_mera = (1 + mesecna_obrestna_mera) ** 12 - 1  # Izračuna letno obrestno mero z obrestno-obrestnim računom.
    odstotki = letna_obrestna_mera * 100                         # Letno obrestno mero pretvori iz decimalnega zapisa v odstotke.

    return round(odstotki)                                       # Vrne odstotke, zaokrožene na najbližje celo število.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjIsInVzZXIiOjExNjI0fQ:1wNAxj:yw0R3W89TvL9a0NVtJZw6hf-2fOjJKN5LdF6baQFaiI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjMsInVzZXIiOjExNjI0fQ:1wNAxj:EcO1Z4PdFTW-dAHqU6PEStgHYYl-n5Xnx1SDxZ0qnHQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MzUsInVzZXIiOjExNjI0fQ:1wNAxj:Qdj32BjMA5_rYHEiTArUTNmJ-ECpJzDBlwmwHMnCaY8"
        try:
