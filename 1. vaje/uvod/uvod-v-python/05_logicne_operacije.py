...
def disjunkcija(a, b):      # Definira funkcijo, ki sprejme dve logični vrednosti.
    return a or b           # Vrne logično disjunkcijo vrednosti a in b.

# Drugi način brez or:

def disjunkcija(a, b):                    # Definira funkcijo, ki sprejme dve logični vrednosti.
    if a == False and b == False:         # Preveri, ali sta obe vrednosti False.
        return False                      # Vrne False, ker je disjunkcija dveh False enaka False.
    return True                           # Vrne True v vseh preostalih primerih.
...
def negacija(a):              # Definira funkcijo, ki sprejme eno logično vrednost.
    return not a              # Vrne nasprotno logično vrednost od a.

# Drugi način:

def negacija(a):              # Definira funkcijo, ki sprejme eno logično vrednost.
    if a == False:            # Preveri, ali je vrednost a enaka False.
        return True           # Vrne True, ker je negacija vrednosti False enaka True.
    return False              # Vrne False v vseh preostalih primerih, torej ko je a enak True.
...
def implikacija(a, b):                  # Definira funkcijo, ki sprejme dve logični vrednosti.
    return not a or b                   # Vrne logično vrednost implikacije a => b.

# Drugi način:

def implikacija(a, b):                  # Definira funkcijo, ki sprejme dve logični vrednosti.
    if a == True and b == False:        # Preveri edini primer, ko je implikacija False.
        return False                    # Vrne False, ker iz True ne sme slediti False.
    return True                         # Vrne True v vseh preostalih primerih.
...
def ekvivalenca(a, b):              # Definira funkcijo, ki sprejme dve logični vrednosti.
    return a == b                   # Vrne True, če sta a in b enaka, sicer False.

# Drug način:

def ekvivalenca(a, b):                                      # Definira funkcijo, ki sprejme dve logični vrednosti.
    if a == False and b == False or a == True and b == True: # Preveri, ali imata a in b enako logično vrednost.
        return True                                         # Vrne True, ker je ekvivalenca resnična, kadar sta vrednosti enaki.
    return False                                            # Vrne False, ker je ekvivalenca neresnična, kadar sta vrednosti različni.
...
def xor(a, b):              # Definira funkcijo, ki sprejme dve logični vrednosti.
    return a != b           # Vrne True, če sta a in b različna, sicer False.

# Drug način:

def xor(a, b):                                        # Definira funkcijo, ki sprejme dve logični vrednosti.
    if a == False and b == False or a == True and b == True:  # Preveri, ali imata a in b enako logično vrednost.
        return False                                  # Vrne False, ker je XOR neresničen, kadar sta vrednosti enaki.
    return True                                       # Vrne True, ker je XOR resničen, kadar sta vrednosti različni.
...
def nand(a, b):                    # Definira funkcijo, ki sprejme dve logični vrednosti.
    return not (a and b)           # Vrne negacijo izraza a and b, kar je NAND.

# Drug način:

def nand(a, b):                    # Definira funkcijo, ki sprejme dve logični vrednosti.
    if a == True and b == True:    # Preveri edini primer, ko je NAND neresničen.
        return False               # Vrne False, ker je NAND napačen, kadar sta a in b oba True.
    return True                    # Vrne True v vseh preostalih primerih.
...
def negacija_nand(a):                         # Definira funkcijo, ki sprejme eno logično vrednost.
    return nand(a, a)                         # Vrne negacijo vrednosti a z uporabo NAND.

def disjunkcija_nand(a, b):                   # Definira funkcijo, ki sprejme dve logični vrednosti.
    return nand(nand(a, a), nand(b, b))       # Vrne disjunkcijo a ali b z uporabo De Morganovega pravila in NAND.

def konjunkcija_nand(a, b):                   # Definira funkcijo, ki sprejme dve logični vrednosti.
    if nand(a, b) == True:                    # Preveri, ali je rezultat NAND za a in b enak True.
        return False                          # Vrne False, ker je konjunkcija False, kadar je NAND True.
    return True                               # Vrne True, ker je konjunkcija True samo takrat, ko je NAND False.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTQsInVzZXIiOjExNjI0fQ:1wNAxj:9SYw8Ca171r2uW-XxtaM7zXtHTGJ7nNH10HF_asZ6Vc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjAsInVzZXIiOjExNjI0fQ:1wNAxj:539Q2ORMwfS7i-SKH3-5t_Kzpr7sGn_nNIOna5iMREQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTUsInVzZXIiOjExNjI0fQ:1wNAxj:Z7jVE8lxUXIWMeUkw8lLM3MuJ0RNHOb6NwI3fdeObqo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTYsInVzZXIiOjExNjI0fQ:1wNAxj:ICpPe-4-mjeJ7AQv7rcKtPbocI1JDIKh43wQpvmCPpY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTcsInVzZXIiOjExNjI0fQ:1wNAxj:fT4NwmP9rWGkiZxxDSh75XZbldNhjYY_V-QSULBXH_A"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTgsInVzZXIiOjExNjI0fQ:1wNAxj:2lkEEFHDYheLPyR3G1QvxEiovZ0FuLi45vFgS36OwSE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MTksInVzZXIiOjExNjI0fQ:1wNAxj:GtW2X1KQZZbShRNyJnY7CEoNh88kXQDia_91myqeWkQ"
        try:
