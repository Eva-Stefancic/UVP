import csv
import os
import requests # to si nalozis na racunalnik: v terminal vpisi: python3 -m pip install requests
import re
...
def download_url_to_string(url): # kodo s spletne strani prenese na racunalnik
...
def page_to_ads(page_content):
    ...
    vrne seznam oglasov."""
    return re.findall('<article class="entity-body cf">(.*?)</article>', page_content, flags=re.DOTALL)
...
def get_dict_from_ad_block(block):
    ...
    if ime == None or lokacija== None or datum == None or cena == None:
        ...
        if "Cena po dogovoru" in cena.group(1):
            ...
            cena = re.search(r'(\d+)&nbsp;<span class="currency">(.)', cena.group(1))
...
def write_cat_ads_to_csv(ads, directory, filename):
    ...
    assert ads and (all(j.keys() == ads[0].keys() for j in ads)) #assert: trdim, da je to res - za preverjanje, da preverja prdpostavke, ali so res
