...
def najpogostejse_besede(vhod, st_besed, izhod):
    ...
    with open(vhod, encoding='utf-8') as f:
        print("Pripravljam besede ...")
        for vrstica in f:
            for beseda in vrstica.lower().strip().replace(',', ' ').replace('.', ' ').split(' '):
                if beseda != "":
                    vse_besede.append(beseda)
                    if beseda not in brez_ponovitev:
                        brez_ponovitev.append(beseda)
    # Seznam pari bo vseboval pare oblike (-3, 'mati'). Druga komponenta bo
    # beseda; prva komponenta bo število pojavitev te besede v seznamu vse_besede, pomnoženo z -1.
    print("Štejem besede ...")
    pari = []
    for beseda in brez_ponovitev:
        pari.append((-vse_besede.count(beseda), beseda))
    ...
    print("Urejam ...")
    pari.sort()
    pari = pari[:st_besed] # Izberemo le prvih st_besed
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODkxLCJ1c2VyIjoxMTYyNH0:1wGF4u:QW7l23wZBbOd0SV_3qVL4FSq76-dJH3HYMyA4MMYPu8"
        try:
            pass
        except TimeoutError:
