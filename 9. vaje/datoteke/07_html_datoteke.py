...
def tabela(vhodna, izhodna):
    with open(vhodna, "r", encoding="utf-8") as vhod:
        with open(izhodna, "w", encoding="utf-8") as izhod:
            ...
            for vrstica in vhod:
                print("  <tr>", file=izhod)
                for podatek in vrstica.strip().split(","):
                    print("    <td>" + podatek + "</td>", file=izhod)
                print("  </tr>", file=izhod)
            print("</table>", file=izhod)
            ...
            for vrstica in vhod:
                if vrstica.startswith("* "):
                    if not seznam:
                        print("<ul>", file=izhod)
                        seznam = True
                    print("  <li>" + vrstica[2:-1] + "</li>", file=izhod)
                else:
                    if seznam:
                        print("</ul>", file=izhod)
                        seznam = False
    ...
    with open(ime_vhodne, "r", encoding="utf-8") as vhod:
        ...
        for vrstica in vhod:
            ...
            if vrstica.strip() != "":
                ...
                nivo = zamik // 2
                besedilo = vrstica.strip()
                elementi.append((nivo, besedilo))

    with open(ime_izhodne, "w", encoding="utf-8") as izhod:
        print("<ul>", file=izhod)

        for i in range(len(elementi)):
            nivo, besedilo = elementi[i]

            print(" " * (4 * nivo + 2) + "<li>" + besedilo, file=izhod)

            if i < len(elementi) - 1:
                naslednji_nivo = elementi[i + 1][0]
            else:
                naslednji_nivo = 0

            if naslednji_nivo > nivo:
                for j in range(nivo + 1, naslednji_nivo + 1):
                    print(" " * (4 * j) + "<ul>", file=izhod)

            elif naslednji_nivo < nivo:
                for j in range(nivo, naslednji_nivo, -1):
                    print(" " * (4 * j) + "</ul>", file=izhod)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTA2LCJ1c2VyIjoxMTYyNH0:1wGF4u:Y1rM0iNgZV5swjrbwISpuiumK20wNTZXKRJbNEvVwZU"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTA3LCJ1c2VyIjoxMTYyNH0:1wGF4u:y83l-H1t8Lm_kR9aaRJpK7_flbjvXDHWS4qX_n3FnvQ"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTA4LCJ1c2VyIjoxMTYyNH0:1wGF4u:fOkvv3U8rkP99J4ldQY0626DKh2IXH1PjjvEiPjqIFA"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTA5LCJ1c2VyIjoxMTYyNH0:1wGF4u:0E8u8z1amRE1uOUDPbfolatPxSV5sa2NvJ24w5CM1kY"
        try:
            test_data = [
