...
def nabor(niz):
    ...
    return tuple(seznam)     # iz seznama v nabor
# =====================================================================@001519=
# 2. podnaloga
...
def nalozi_csv(datoteka):
    rezultat = []
    with open(datoteka, "r", encoding = "utf-8") as vhod:
        for vrstica in vhod:
...
def vsote(vhodna, izhodna):
    ...
    with open(izhodna, "w", encoding = "utf-8") as izhod:
        for student in podatki:
            print(student[0], sum(student[1:]), sep = ",", file=izhod)
...
def rezultati(vhodna, izhodna):
    podatki = nalozi_csv(vhodna)
    podatki.sort(key=lambda student: student[0].split()[1])               #urejeno bo po imenih, želimo pa si po priimkih
    with open(izhodna, "w", encoding="utf-8") as izhod:
        for student in podatki:
            print(*student, sum(student[1:]), sep = ",", file = izhod)    # *(seznam) pomeni, da poda vse podatke ime, priimek, točke .. 

        izhod.write("POVPRECEN STUDENT")
        skupna_vsota = 0
        for i in range(1, len(podatki[0])):
            vsota = 0
            for student in podatki:
                vsota += student[i]
            skupna_vsota += vsota
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTE4LCJ1c2VyIjoxMTYyNH0:1wGF4u:CwTm529BGa9ofgTLDV_CGdu2uKOM6J0qR1TJ3dezeho"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTE5LCJ1c2VyIjoxMTYyNH0:1wGF4u:rijI1h1E2LLRCSedMxvuTyTdr26NPXEDikWPcOxR6DU"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTIwLCJ1c2VyIjoxMTYyNH0:1wGF4u:EQOLhELLzs5jubzGyd2gUtg24KGxogUL98ORL-1mSlI"
        try:
            test_data = [
                ("kolokviji_vhod.txt", ["Janez Novak,1,3,3,2,0", "Micka Kovaceva,0,3,2,3", "Miha Praznic", "Peter Klepec,1,0,1,2,1,3"],
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxNTIxLCJ1c2VyIjoxMTYyNH0:1wGF4u:vzyuqIw82CkBdskb8p96JW9fjaFaq4IaYOFIBk8Pojc"
        try:
            test_data = [
