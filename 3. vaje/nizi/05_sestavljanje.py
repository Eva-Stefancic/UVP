...
def pozdrav(ime, priimek):
    ...
    return (f"Lep pozdrav! " + ime.title() + " " + priimek.title() + ".") 
                # .title vsako ime in priimkek (če jih je več) začne z veliko začetnico
...
def zmnozi(n, m):
    zmnozek = ""
    zmnozek = float(n) * float(m)  # dobim število
    return f"{zmnozek:.7f}"        # število pretvorim nazaj v niz in vzamem le 7 decimalnih mest
        # zakaj pretvorim nazaj v niz? ker se pri float pobrišejo ničle pri npr 83,2700000
...
def turist(stevilka_racuna, cena, davek_odstotki):  
    stevilka_racuna = str(stevilka_racuna)          # Številko računa pretvorimo v niz, da ji lahko dodajamo ničle.
    nicle = int(7 - (len(stevilka_racuna))) * "0"   # Izračunamo, koliko ničel manjka do dolžine 7, in jih ustvarimo.
    stevilka_racuna_7 = nicle + stevilka_racuna     # Ničle dodamo pred številko računa.

    ddv = cena * davek_odstotki//100                # Izračunamo znesek DDV-ja.
    vrednost = cena + ddv                           # Izračunamo skupno vrednost računa z DDV-jem.
    return f"-RACUN---" + str(stevilka_racuna_7) + "\nCena: " + str(cena) + " EUR\nDDV: " + str(ddv) + "\nSkupno: " + str(vrednost) + "\nHvala za obisk"  # Sestavimo in vrnemo celoten račun kot niz.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5MywidXNlciI6MTE2MjR9:1w0kVp:OjoorBOFDHYLd2LJ8-lGD-VJEejvd0cfF1EavH8q8go"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NCwidXNlciI6MTE2MjR9:1w0kVp:MDcc0WKYIIkThdDbIAABiVEB2rJodks-5Jn0_n1DZac"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NSwidXNlciI6MTE2MjR9:1w0kVp:xnxV8nXaDG17aneDyuLI8qsBagJDbGYrs56orG0nF5o"
        try:
