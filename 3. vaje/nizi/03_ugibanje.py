...
def preberi_celo_stevilo():                                           
    niz = input("> Vnesi celo število: ")                             # Uporabnika prosi za vnos in ga shrani kot niz.
    if niz.isnumeric() or (niz != "" and niz[0]== "-" and niz[1:].isnumeric()):  # Preveri, ali je vnos pozitivno ali negativno celo število.
        return int(niz)                                               # Veljaven niz pretvori v celo število in ga vrne.
    else:                                                            # Izvede se, če vnos ni veljavno celo število.
        print (f'Žal "{niz}" ni celo število, poskusi ponovno!')      # Izpiše obvestilo, da vnos ni veljaven.
        return preberi_celo_stevilo()                                 # Ponovno pokliče funkcijo in zahteva nov vnos.
...
def ugibaj(niz):
    inputniz = input("> Vnesi celo število: ")
    if inputniz.isnumeric() or inputniz[0]== "-" and inputniz[1:].isnumeric():
        vnos = int(inputniz)
        if vnos < niz:
            ...
            print(f"BRAVO! Res sem si zamislil število {niz}!")

    else:
        print(f'Žal "{inputniz}" ni celo število, poskusi ponovno!')
        ugibaj(niz)


# kot zahteva naloga je spodaj rešitev, ki pokliče prejšnjo funkcijo

def ugibaj(niz):                                             # Definira funkcijo, ki sprejme pravilno število.
    vnos = preberi_celo_stevilo()                            # Pokliče prejšnjo funkcijo in dobi veljavno celo število.
    if vnos < niz:                                           # Preveri, ali je vneseno število manjše od pravilnega.
        print('Moje število je večje!')                      # Izpiše, da je iskano število večje.
        ugibaj(niz)                                          # Ponovno pokliče funkcijo za nov poskus.
    elif vnos > niz:                                         # Preveri, ali je vneseno število večje od pravilnega.
        print('Moje število je manjše!')                     # Izpiše, da je iskano število manjše.
        ugibaj(niz)                                          # Ponovno pokliče funkcijo za nov poskus.
    else:                                                    # Izvede se, če je uporabnik uganil pravilno število.
        print(f"BRAVO! Res sem si zamislil število {niz}!")  # Izpiše sporočilo ob pravilnem odgovoru.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NiwidXNlciI6MTE2MjR9:1w0kVp:8rkvHQTA8SFyNoiJmH48jLvLb03IwBo-Rufv5pZeDpo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5NywidXNlciI6MTE2MjR9:1w0kVp:hBpdsZI8jsAj6hQ7xt-AJ3jdbg44hKzYQSajzfAaP5A"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzYyMywidXNlciI6MTE2MjR9:1w0kVp:RFDmYSENANcN53WqVsLtt8-WGaOCQzlqqL5kPe4VlL8"
        try:
