...
def prezrcali(niz):
    return niz[::-1]   # niz[zacetek:konec:korak]
                     # Vrne niz od zadaj naprej; -1 pomeni premikanje po znakih v obratni smeri.
...
def je_palindrom(niz):
    return niz == prezrcali(niz)  # Niz od spredaj je enak nizu od zadaj
...
def odstrani_samoglasnike(niz):
    return niz.lstrip("aeiouAEIOU")  # Z začetka niza odstrani navedene samoglasnike.
                                     # lstrip() odstranjuje samo z začetka, ne iz sredine ali konca.
...
def obrni_oklepaje(niz):
    return niz.replace("(", "x").replace(")", "(").replace("x", ")")
                                # Najprej začasno zamenja (, nato obrne ), na koncu pa x spremeni v ).
                                # x je tu varna izbira umesne spremenljivke, saj niz sprejema samo števila in operatorje.
...
def prestej_posebno(niz, c, k):                  # Definira funkcijo, ki sprejme niz, znak c in število k.
    if k == 0:                                   # Preveri, ali smo že prišli do iskane, k-te pojavitve znaka c.
        return niz.count(" ")                    # Prešteje vse presledke v preostalem delu niza.
    else:                                        # Izvede se, če moramo še iskati naslednjo pojavitev znaka c.
        i = niz.find(c)                          # Poišče indeks prve pojavitve znaka c v trenutnem nizu.
        if i < 0:                                # Preveri, ali znaka c v trenutnem nizu ni več.
            return 0                             # Če znaka c ni več, vrne 0.
        else:                                    # Izvede se, če je bil znak c najden.
            return prestej_posebno(niz[i+1:], c, k-1)  # Nadaljuje z delom niza za najdenim znakom in zmanjša k za 1.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5MSwidXNlciI6MTE2MjR9:1w0kVp:xo76H-CI5F7Avh4G2dB0oLdFfiA7J2_NIfyFDHQ68uQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5MiwidXNlciI6MTE2MjR9:1w0kVp:uIY1BWo6bH3k83qHlLQ0SKC1mpZ5Ur4Qo5JBkSzPxgw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4MywidXNlciI6MTE2MjR9:1w0kVp:R9xlmBNcUH8sUQ_Dhv_BLBB2TlCz8Y0qphE5xT9BIrc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4NCwidXNlciI6MTE2MjR9:1w0kVp:2y94v_zUTA2htO0MYi6WcCg5TxQsIyZj6IovhcNiFYY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4NSwidXNlciI6MTE2MjR9:1w0kVp:PRZzgQF4cq3_cZ2IONHwPljvvH-X0MheREdRJw43c9g"
        try:
